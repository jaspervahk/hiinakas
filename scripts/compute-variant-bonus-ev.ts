...
let rBase: Record<BonusQualifier, number> | null = null
if (N_RBASE > 0) {
  const r2 = mulberry32(99881)
  rBase = { QQ: 0, KK: 0, AA_OR_TRIPS: 0 }
...
// Emit the table from the heuristic distribution, which is the one consistent
// with how R[a][BASE] and the sample pools were generated.
const { S, x } = solve(heur.P, heur.q)
...
console.log(`// bonus round when their side game qualifies (q = ${heur.q.toFixed(5)}).`)
