...
function CardChip({ c }: { c: Card }) {
  return (
    <span className={`inline-block font-mono text-xs ${SUIT_COLOR[c.suit] ?? 'text-slate-200'}`}>
      {cl(c)}
    </span>
  )
}
...
function RunningChart({ summaries, p1, p2 }: { summaries: GameSummary[]; p1: string; p2: string }) {
  ...
  // Show p2 (jass) perspective so the chart reads as "my running score"
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 140 }}>
      ...
      {summaries.map((s, i) => {
        if (s.p1Bust || s.p2Bust) return null
        return (
          <circle key={i} cx={toX(i + 1)} cy={toY(s.p2Run)} r="1.5" fill="#818cf8" />
        )
      })}

      {/* X-axis: show game numbers every 5 */}
      {summaries.map((_, i) => {
        const n = i + 1
        if (n !== 1 && n % 5 !== 0 && n !== summaries.length) return null
        return (
          <text key={i} x={toX(i + 1)} y={H - 4} textAnchor="middle" fontSize="7" fill="#4b5563">
            {n}
          </text>
        )
      })}
...
function buildAnalyzed(
  ...
  return decisions.flatMap(d => {
    ...
    return [{ ...d, candidates, bestPlacement: best.placement, playedEV, bestEV, evLost: bestEV - playedEV }]
  })
}
...
function BlunderCard({ d, rank }: { d: AnalyzedDecision; rank: number }) {
    ...
    <div className="bg-gray-900 rounded-lg p-3 border border-gray-800">
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className="flex gap-1">{d.infoState.hand.map((c, i) => <CardChip key={i} c={c} />)}</span>
        </div>
...
function GameLogTable({
  ...
  const evByGame = useMemo(() => {
    const map = new Map<string, { p1: number; p2: number; p1Count: number; p2Count: number }>()
    for (const d of analyzed) {
      const cur = map.get(d.gameId) ?? { p1: 0, p2: 0, p1Count: 0, p2Count: 0 }
      if (d.username === p1) { cur.p1 += d.evLost; cur.p1Count++ }
      else { cur.p2 += d.evLost; cur.p2Count++ }
      ...
      <table className="w-full text-xs border-collapse">
        <thead>
          <tr className="text-gray-500 border-b border-gray-800">
            ...
            {hasEV && <>
              <th className="text-right py-1.5 px-2 font-normal text-indigo-400/70">EV lost</th>
              <th className="text-right py-1.5 px-2 font-normal text-amber-400/70">EV lost</th>
            </>}
          </tr>
        </thead>
        <tbody>
          {summaries.map((s, i) => {
            ...
            return (
              <tr key={s.gameId}
                ...
                {hasEV && <>
                  <td className="py-1 px-2 text-right font-mono text-indigo-400/70">
                    {ev ? `-${ev.p1.toFixed(1)}` : '—'}
                  </td>
                  <td className="py-1 px-2 text-right font-mono text-amber-400/70">
                    {ev ? `-${ev.p2.toFixed(1)}` : '—'}
                  </td>
                </>}
              </tr>
...
function SessionTabInner() {
  ...
  const handleFile = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setRawData(parsed)
        // Auto-select the highest-count pair
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      p1Wins, p2Wins, ties,
      p1Total: final.p1Run, p2Total: final.p2Run,
      p1SoloBusts, p2SoloBusts, bothBusts,
      p1BustCost, p2BustCost,
    }
  }, [summaries])

  // Run EV analysis — loads model first if not already in worker
  const runAnalysis = useCallback(async () => {
    if (decisions.length === 0) return
    setAnalyzing(true)
    setNoModel(false)
    try {
      // Try loading the model if not already loaded (handles the case where
      // initial app-startup load failed because the model wasn't deployed yet)
      ...
      if (results.length > 0 && !results[0]!.hasModel) {
        // Model not in worker — attempt a fresh load then retry once
    ...
    return (
      <div className="space-y-4 py-8">
        ...
        <div className="flex flex-col gap-2 max-w-sm mx-auto">
          {pairs.map(p => (
            <button key={`${p.p1}|${p.p2}`}
              onClick={() => setPair([p.p1, p.p2])}
        ...
        <div className="text-center">
          <button onClick={() => { setRawData(null); setPair(null) }}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setPair(null); setAnalyzed([]) }}
          className="text-gray-600 hover:text-gray-400 text-xs">
          ← Change
        </button>
      </div>
      ...
      <div className="space-y-3">
        ...
        {noModel && (
          <div className="flex items-center justify-between bg-amber-900/20 rounded px-3 py-2">
            ...
            <button
              onClick={runAnalysis}
              className="ml-3 shrink-0 px-2 py-1 text-xs rounded bg-amber-800/40 hover:bg-amber-700/40 text-amber-300 transition-colors"
            >
              Retry
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              {blunders.p1.slice(0, 5).map((d, i) => (
                <BlunderCard key={d.id} d={d} rank={i + 1} />
              ))}
            </div>
            {/* p2 blunders */}
            <div className="space-y-2">
      ...
      {/* Parse error */}
      {parseError && (
        <p className="text-red-400 text-xs bg-red-900/20 rounded px-3 py-2">
      ...
      {summaries.length > 0 && (
        <div className="space-y-2">
          ...
          <GameLogTable summaries={summaries} p1={p1} p2={p2} analyzed={analyzed} />
        </div>
      )}
    </div>
  )
